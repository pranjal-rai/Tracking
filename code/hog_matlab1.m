...
samples1=zeros(n_pos_samples,15*7*36);
...
while i<=n_pos_samples
    ...
    hog = hog_feature_vector(im);
    descriptor=hog(:);
    samples1(i,:)=descriptor(:);
%     scores=w'*descriptor+b;
%     pos(i)=scores;
    i=i+1;
end
pos=svmclassify(svmstruct,samples1);
neg=zeros(n_neg_samples,1);
samples2=zeros(n_neg_samples,15*7*36);
path='/home/gazzib/Downloads/honours/Tracking/Tracking dataset/negative samples/';
i=1;
while i<=n_neg_samples
    fname=strcat(path,strcat(int2str(i),'.jpg'));
    ...
    hog = hog_feature_vector(im);
    descriptor=hog(:);
    samples2(i,:)=descriptor(:);
%     scores=w'*descriptor+b;
%     neg(i)=scores;
    i=i+1;
end
neg=svmclassify(svmstruct,samples2);
% 
